...
| Contextual Retrieval (실패) | 규정·매뉴얼에 LLM 맥락 prefix | 68.4% | 65.8% | 81.7% | 100% |

**시스템 누적 +15.8%p retrieval@k**, faithfulness 100% 유지.

### 골든셋 v2 정밀화 (25문항)

같은 시스템(B4)을 정밀화된 골든셋으로 측정:

| 골든셋 | retrieval@k | recall | kw recall | faithfulness |
|---|---|---|---|---|
| v1 (20문항, 일부 expected 부정확) | 68.4% | 65.8% | 82.5% | 100% |
| **v2 (25문항, 정밀화)** | **87.5%** | 74.3% | 82.0% | 100% |

**+19.1%p**는 시스템 개선이 아닌 **측정 정확도 향상**. v1에서 expected_sources가 너무 좁게 잡혀 있던 케이스가 시스템의 실제 정답을 ❌로 분류하던 문제 해소.

## 실패한 실험 기록

다시 시도하지 않도록 명시:

### Contextual Retrieval — Anthropic 기법 (2026-04-30)

**가설**: 각 청크에 LLM이 만든 50자 맥락 prefix를 추가하면 검색 적중률 향상 (Anthropic 블로그 주장: 검색 실패 49% 감소).

**적용 범위**: 규정·매뉴얼 청크 439건 (Q&A 제외).

**결과 (golden v1 기준)**: retrieval@k 68.4% → 68.4% (변화 없음). q02·q16 ✅ 개선됐으나 q07·q19 ❌ 퇴보로 상쇄.

**원인**: 우리 baseline은 이미 hybrid + multi-query rewriting이 작동 중. Anthropic 주장은 dense-only baseline 기준일 가능성. 또한 prefix가 매뉴얼 청크의 매칭력을 강화해 다른 도메인(Q&A, 자체규정)의 정답을 밀어내는 부작용.

**처리**: prefix는 그대로 둠 (무해, 향후 활용 가능). 비용: ~$0.10 + 8분 시간.

**교훈**: 다른 시스템에서 큰 효과를 본 기법도 우리 baseline 구성에 따라 marginal일 수 있음. 여러 기법을 동시 도입하면 효과가 서로 잠식.

---

### kiwipiepy 형태소 분석 BM25 토크나이저 (2026-04-30)

**가설**: 한국어는 교착어라 조사("장학금은", "장학금이")가 분리 안 되면 BM25 recall 손실. 형태소 분석으로 명사·어근만 추출하면 매칭 향상.

**결과**: retrieval@k **68.4% → 57.9% (−10.5%p 퇴보)**.

**원인**: 우리 도메인 핵심 키워드의 대다수가 복합명사 ("신진연구인력", "교육연구단", "참여대학원생", "종합정보관리시스템"). kiwipiepy가 이들을 ["신진","연구","인력"] 식으로 분해해 IDF 분산 → 정확 매칭의 변별력이 사라짐. 조사 분리 이득보다 복합명사 분해 손실이 큼.

**교훈**: "한국어 = 형태소 분석" 통념이 모든 도메인에 맞지 않음. **고정 복합명사가 핵심 어휘인 정부·기술 도메인**에서는 default tokenizer(공백+특수문자 분리)가 더 효과적.

**재시도 조건**: 인접 NN* 명사를 결합해 통째 토큰으로도 보존하는 후처리(예: "신진연구인력" → ["신진연구인력","신진","연구","인력"] 둘 다 인덱싱)를 추가하면 재시도 가치 있음. 단, 인덱스 크기 증가와 점수 분포 변화로 다른 부작용 발생 가능.
